# 🧭 SENSORYSCOPE: PROJECT PURPOSE & AGENT DIRECTIVES
...
**The UX Mandate: "Calm Minimalist"**
- **Zero Friction:** The user must be able to find a "Safe Haven" in 2 taps or less.
- **No Aggressive UI:** Do NOT use harsh colors (like pure `#FF0000`). Use the provided soft, pastel-leaning design system. 
...
- **Maps:** `@rnmapbox/maps` + `@rneui/themed`
- **UI/Styling:** `@rneui/themed` + custom `DesignSystem.ts` (using `StyleSheet.create`).
- **Icons:** `react-native-vector-icons` (or Lucide Icons if specifically requested).
...
When generating code, modifying files, or debugging, the AI Agent MUST follow these rules:
...
This app is being built for the *Ready, Set, App!* competition in North Carolina. The code must be clean, well-commented, and pitch-ready. Features that demonstrate technical complexity (like real-time DB triggers, geolocation, and hardware microphone access) should be prioritized and highlighted in the code structure.